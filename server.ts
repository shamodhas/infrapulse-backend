...
const clientAuthMiddleware = (
  ...
  const clientToken = authHeader ? authHeader.split(" ")[1] : undefined
  if (!clientToken)
  ...
  async (req: express.Request, res: express.Response): Promise<any> => {
    try {
      ...
      if (!targetHost || !sshSecret) {
        return res
          .status(400)
          .json({ error: "Target host or SSH credentials missing in headers" })
      }
      ...
      return res
        .status(500)
        .json({ error: `SSH Connection Failed: ${error.message}` })
    }
  }
)
    ...
    try {
      ...
      if (!targetHost || !sshSecret) {
        return res
          .status(400)
          .json({
            error: "Target host or SSH credentials missing in request body"
          })
      }
      ...
      return res
        .status(500)
        .json({ error: `Failed to restart container: ${error.message}` })
    }
  }
)
